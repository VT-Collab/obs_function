...
class MiniGridEnv(gym.Env):
    ...
    def __init__(
        ...
        agent_fov: int = 90, #new MISHA EDIT
    ...
    #new Misha edit class REDOOOOO
    #overcooked fov in state_visualizer.py -> search grid
    #given world coordinates (x,y), decide whether the tile there stops vision eg. like whether it is an empty grid/open door or a wall
    #returns true if  world tile at (x,y) blocks vision.
    def _blocks_vision(self, x: int, y: int) -> bool:
        #get the given tile, if empty it does not block vision
        tile = self.grid.get(x, y)
        if tile is None:
            return False
        
        #Rules: if open door, False, otherwise if closed door or wall, True
        #get attribute
        t = getattr(tile, "type", None)
        #if door 
        if t == "door":
            #if open door, false
            if getattr(tile, "is_open", False):
                return False 
            #if closed door, true
            else:
                return True
    ...
    #x0, y0 = agent world tule
    #x1, y1 = target tile the one we try to figure out can see or not
    #this function enumeriates grid cells along straight line froma gent to target tile
    #Yield each cells from the path from (x0,y0) to (x1,y1) inclusive, and like as staright as possible in a diagonal line
    ...
    def _bresenham_line(self, x0: int, y0: int, x1: int, y1: int):
        ...
        #step directions for moving left/right/ down/up
        sx = 1 if x0 < x1 else -1
        sy = 1 if y0 < y1 else -1
        
        #how much error or how much we deviate from the ideal line; initialize as the distances 
        err = dx + dy
        ...
        while True:
            ...
            #if too much error on y, we do y. if too much on x, we do x
            e2 = 2 * err
            if e2 >= dy:
                err += dy; x += sx
            if e2 <= dx:
    ...
    #walks from agent to target cell and see if blocking
    #returns true if no intermediate blockers between start and target
    #
    def _los_clear(self, x0: int, y0: int, x1: int, y1: int) -> bool:
        first = True
        
        #walks every cell in the "straight line"
        for x, y in self._bresenham_line(x0, y0, x1, y1):
            ...
            if x == x1 and y == y1:
                return True
            #if hit a vision blocking cell, return false
            if self._blocks_vision(x, y):
                return False
        
        return True

    #builds booleen visibility mask over the entire world, like a cone that uses cosine test
    #if cell is inside forward cone, go to LOS test and see if vision is blocked in the middle during the path using BH line that acts like a ray
    #returns boolean visibility mask over entire world grid, WxH boolean array where True = cell is visible
    def _world_cone_vis_mask(self) -> np.ndarray:
    ...
    """same world mask logic but with a passable fov just for the estimated case"""
    def estimated_world_cone_vis_mask(self, fov) -> np.ndarray:
    ...
    def get_pov_render(self, tile_size):
        """
        Render the cone FOV in the same way as full render (single source of truth).
        """
        return self.get_full_render(True, tile_size)
    ...
    def get_full_render(self, highlight, tile_size):
        """
        Render a non-partial observation with a world-space cone FOV highlight.
        """
        highlight_mask = self._world_cone_vis_mask()  # shape (W, H), indexed [x, y]
