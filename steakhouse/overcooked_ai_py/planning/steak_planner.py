...
from steakhouse_ai_py.mdp.steakhouse_mdp import (
    EVENT_TYPES,
    SteakhouseGridworld,
    SteakhouseState,
    PlayerState,
...
class JointMotionPlanner(object):
    ...
    def __init__(self, mdp, params, debug=False):
        ...
        # Turn on/off same cell occupation for agents
        self.enable_same_cell = params["enable_same_cell"]
    ...
    def _obtain_plan(self, joint_start_state, joint_goal_state):
        ...
        if not have_conflict or self.enable_same_cell:
            (
                joint_action_plan,
    ...
    def is_valid_joint_motion_goal(self, joint_goal_state):
        ...
        if multi_cc_map and players_in_same_cc and not self.enable_same_cell:
            return False
        return all(
            [
...
class MediumLevelActionManager(object):
    ...
    def pickup_onion_actions(self, counter_objects, only_use_dispensers=False):
        ...
        if not only_use_dispensers and ("onion" in counter_objects.keys()):
    ...
    def pickup_tomato_actions(self, counter_objects):
        ...
        if "tomato" in counter_objects.keys():
            tomato_pickup_locations += counter_objects["tomato"]
    ...
    def pickup_dish_actions(self, counter_objects, only_use_dispensers=False):
        ...
        if not only_use_dispensers and ("dish" in counter_objects.keys()):
    ...
    def pickup_counter_soup_actions(self, counter_objects):
        if "soup" in counter_objects.keys():
    ...
    def place_obj_on_any_empty_counter_actions(self, state):
        all_empty_counters = set(self.mdp.get_empty_counter_locations(state))
        return self._get_ml_actions_for_positions(all_empty_counters)
    ...
    def _get_ml_actions_for_positions(self, positions_list, avoid_pos=[]):
        ...
        for pos in positions_list:
            ...
            potential_motion_goal = self.joint_motion_planner.motion_planner.motion_goals_for_pos[pos]
            # avoid_pos containts the other agent's position. If the other agent is already standing at potential motion goals, we assume they are already performing the task, therefore, avoid adding that task to our motion goal.
            for a in avoid_pos:
                if a in potential_motion_goal:
                    avoid_flag = True
                    break
...
class SteakMediumLevelActionManager(MediumLevelActionManager):
    ...
    def from_pickle_or_compute(
        ...
        if force_compute:
            return SteakMediumLevelActionManager.compute_mlam(
                filename, mdp, mlam_params, info=info
            )
        ...
        if info:
            print(
                "Loaded SteakMediumLevelActionManager from {}".format(
                    os.path.join(PLANNERS_DIR, filename)
                )
            )
        return mlam
    ...
    def compute_mlam(filename, mdp, mlam_params, info=False):
        ...
        if info:
            print(
                "Computing SteakMediumLevelActionManager to be saved in {}".format(
                    final_filepath
                )
            )
        start_time = time.time()
        mlam = SteakMediumLevelActionManager(mdp, mlam_params=mlam_params)
        if info:
            print("It took {} seconds to create mlam".format(time.time() - start_time))
        mlam.save_to_file(final_filepath)
        return mlam
    ...
    def pickup_meat_actions(self, counter_objects):
        ...
        meat_pickup_locations = meat_dispenser_locations
        if "meat" in counter_objects.keys():
            meat_dispenser_locations += counter_objects["meat"]
    ...
    def pickup_chicken_actions(self, counter_objects, knowledge_base=None):
        ...
        chicken_pickup_locations = chicken_dispenser_locations
        if "chicken" in counter_objects.keys():
            chicken_pickup_locations += counter_objects["chicken"]
        return self._get_ml_actions_for_positions(chicken_pickup_locations)

    def chop_onion_on_board_actions(self, state, knowledge_base=None, other_players=[]):
        full_boards = []
        ...
        return self._get_ml_actions_for_positions(full_boards, [o.pos_and_or for o in other_players])

    def pickup_dirty_plate_actions(self, counter_objects, only_use_dispensers=False):
        ...
        if not only_use_dispensers and ("dirty_plate" in counter_objects.keys()):
    ...
    def rinse_plate_in_sink_actions(self, state, knowledge_base=None, other_players=[]):
        rinse_needed_loc = []
        if knowledge_base is not None:
        ...
        return self._get_ml_actions_for_positions(rinse_needed_loc, [o.pos_and_or for o in other_players])
        ...
        if knowledge_base is not None:
            for obj_id, obj in knowledge_base.items():
                ...
                if len(clean_plate_loc) == 0:
                    for obj_id in knowledge_base["sink_states"]["full"]:
                        clean_plate_loc += [knowledge_base[obj_id].position]
                if len(clean_plate_loc) == 0:
                    robot_obj = (
            ...
            if "clean_plate" in counter_objects.keys():
    ...
    def put_meat_in_grill_actions(self, grill_states_dict, knowledge_base=None, only_empty=False):
    ...
    def put_chicken_in_pot_actions(self, pot_states_dict, knowledge_base=None, only_empty=False):
    ...
    def pickup_counter_steak_actions(self, counter_objects, knowledge_base=None):
        if "steak" in counter_objects.keys():
    ...
    def pickup_counter_onion_steak_actions(self, counter_objects, knowledge_base=None):
        if "steak_onion" in counter_objects.keys():
    ...
    def add_garnish_to_dish_actions(self, state, knowledge_base=None):
        garnish_chopped_loc = []
        if knowledge_base is not None:
    ...
    def go_to_closest_feature_actions(self, player):
        feature_locations = (
            ...
            + self.mdp.get_chicken_dispenser_locations()
