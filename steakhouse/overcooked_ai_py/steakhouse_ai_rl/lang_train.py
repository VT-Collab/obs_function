...
class RolloutWorker:
    ...
    def set_agent(self, agent_state_dict):
        ...
        self.agent = LangStayAgent(
            ...
            debug=False,
            device=self.device,
            utter_model_state_dict=agent_state_dict,
            lstm_size=self.args.lstm_size,
            lstm_layers_n=self.args.lstm_layers_n,
    ...
    def rollout(self):
        ...
        next_hist_utter_id = self.agent.hist_utter_id.unsqueeze(0).to(device)
        next_done = torch.zeros(1).to(device)
        next_lstm_state = (
            torch.zeros(self.agent.utter_model.lstm.num_layers, 1, self.agent.utter_model.lstm.hidden_size).to(device),
            torch.zeros(self.agent.utter_model.lstm.num_layers, 1, self.agent.utter_model.lstm.hidden_size).to(device),
        )
        ...
        hist_utter_ids = torch.zeros(((args.num_steps,) + (self.args.hist_utter_size,)), device=device)
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                subtask, logprob, _, value, next_lstm_state = self.agent.utter_model.get_action_and_value(next_obs, next_lstm_state, next_hist_utter_id, next_done)
                values[step] = value.flatten()
            actions[step] = subtask
            logprobs[step] = logprob

            # TRY NOT TO MODIFY: execute the game and log data.
            self.agent.update_utter(subtask.cpu().numpy()[0])
            if isinstance(self.agent, LangStayAgent):
            ...
            hist_utter_ids[step] = self.agent.hist_utter_id.unsqueeze(0).to(device)
        ...
        with torch.no_grad():
            next_value = self.agent.utter_model.get_value(
                next_obs,
                next_lstm_state,
                next_hist_utter_id,
                next_done,
...
if __name__ == "__main__":
    ...
    args.single_action_space_n = args.utter_id_offset + (args.condition_length * args.ml_action_n)
    ...
    human_agent = LangStayAgent(
        ...
        lstm_layers_n=args.lstm_layers_n,
    ...
    next_lstm_state = (
        torch.zeros(human_agent.utter_model.lstm.num_layers, args.num_envs, human_agent.utter_model.lstm.hidden_size).to(device),
        torch.zeros(human_agent.utter_model.lstm.num_layers, args.num_envs, human_agent.utter_model.lstm.hidden_size).to(device),
    )  # hidden and cell states (see https://youtu.be/8HyCNIVRbSU)
    total_reward = 0

    layout_names = ["Overcooked2_1-2"]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        agent_state_dict = {k: v.cpu().clone() for k, v in human_agent.utter_model.state_dict().items()}
        ...
        rollout_hist_utter_ids = torch.stack([r["hist_utter_ids"] for r in rollouts], dim=1)
        ...
        b_hist_utter_ids = rollout_hist_utter_ids.reshape((-1,) + (args.hist_utter_size,)).to(device)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                _, newlogprob, entropy, newvalue, _ = human_agent.utter_model.get_action_and_value(
                    b_obs[mb_inds],
                    (initial_lstm_state[0][:, mbenvinds], initial_lstm_state[1][:, mbenvinds]),
                    b_hist_utter_ids[mb_inds],
                    b_dones[mb_inds],
                ...
                loss.backward()
                nn.utils.clip_grad_norm_(human_agent.utter_model.parameters(), args.max_grad_norm)
        ...
        if args.track and iteration % 100 == 0:
            ...
            torch.save(human_agent.utter_model.state_dict(), f"{wandb.run.dir}/agent.pt")
