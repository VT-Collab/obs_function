...
class RolloutWorker:
    ...
    def rollout(self):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                (id_action, length_action), id_logprob, length_logprob, _, _, value = self.agent.notification_policy.get_action_and_value(next_latent_input)
                values[step] = value.flatten()
            id_actions[step] = id_action
            length_actions[step] = length_action
            logprobs[step] = id_logprob + length_logprob
            start_new_flag = True

            # TRY NOT TO MODIFY: execute the game and log data.
            for _ in range(length_action.item()+buffer_length):
                if next_done.item():
                    break

                if start_new_flag:
                    self.agent.update_utter(notify_mode=1, condition=0, subtask=id_action.item(), length=length_action.item()+2)
                    start_new_flag = False
                else:
                    self.agent.update_utter(notify_mode=2, condition=-1, subtask=0, length=0)
        ...
        with torch.no_grad():
            next_value = self.agent.notification_policy.get_value(
                next_latent_input,
            ).reshape(1, -1)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # manager_agent_state_dict = {k: v.cpu().clone() for k, v in human_agent.manager_policy.state_dict().items()}
        ...
        b_advantages = rollout_advantages.reshape(-1).to(device).detach()
        b_returns = rollout_returns.reshape(-1).to(device).detach()
        b_values = rollout_values.reshape(-1).to(device).detach()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                _, newidlogprob, newlengthlogprob, id_entropy, length_entropy, newvalue = human_agent.notification_policy.get_action_and_value(
                    b_latent_inputs[mb_inds],
                    b_id_actions.long()[mb_inds],
                    b_length_actions.long()[mb_inds],
                )
                newlogprob = newidlogprob + newlengthlogprob
                ...
                entropy_loss = id_entropy.mean() + length_entropy.mean()
                loss = pg_loss - args.ent_coef * entropy_loss + v_loss * args.vf_coef

                human_agent.optimizer.zero_grad()
                loss.backward()
                nn.utils.clip_grad_norm_(human_agent.notification_policy.parameters(), args.max_grad_norm)
        ...
        if args.track and iteration % 100 == 0:
            ...
            torch.save(human_agent.notification_policy.state_dict(), f"{wandb.run.dir}/agent.pt")
