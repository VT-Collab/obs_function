...
from util import make_env, init_env
...
def evaluate(
    ...
    for i in range(eval_episodes):
        ...
        human_agent = HRLModel(mlam, study_config.base_env.state, auto_unstuck=True, explore=cfg.agent.EXPLORE, vision_limit=cfg.agent.VISION_LIMIT, vision_bound=cfg.agent.VISION_BOUND, kb_update_delay=cfg.agent.KB_UPDATE_DELAY, kb_ackn_prob=cfg.agent.KB_ACKN_PROB, obs_size=math.prod(single_observation_space), debug=True)
...
def main(cfg: DictConfig):
    ...
    human_agent = HRLModel(mlam, env.state, auto_unstuck=True, explore=cfg.agent.EXPLORE, vision_limit=cfg.agent.VISION_LIMIT, vision_bound=cfg.agent.VISION_BOUND, kb_update_delay=cfg.agent.KB_UPDATE_DELAY, kb_ackn_prob=cfg.agent.KB_ACKN_PROB, obs_size=math.prod(single_observation_space), debug=True)
    other_agent = StayAgent()
    human_agent.set_agent_index(1)
    other_agent.set_agent_index(0)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = human_agent.subtask_planner.get_action_and_value(b_obs[mb_inds], b_actions.long()[mb_inds])
