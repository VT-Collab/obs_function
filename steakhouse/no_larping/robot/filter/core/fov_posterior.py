...
lives in qmdp.py, which reads the posterior this file maintains and searches
over how to execute its baseline's job against it.
...
That filter is gone: with the target cell living inside a sub-task it could only
choose among cells its baseline had already shortlisted, and it had no way to say
...
class FOVPosterior:
    ...
    def beliefs_for(self, fov):
        ...
        qmdp.py clones the WHOLE shadow rather than calling this, because the
