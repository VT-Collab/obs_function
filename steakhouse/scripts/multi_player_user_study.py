...
class OvercookedPygame():
    ...
    def on_init(self):
        ...
        self.state_visualizer = SteakhouseStateVisualizer(
            headless=self.headless, **test_dict["config"])
        self._running = True
    ...
    def on_render(self):
        kitchen = self.state_visualizer.render_state(
            self.env.state, self.env.mdp.terrain_mtx, hud_data=self.state_visualizer.default_hud_data(
                self.env.state,time_left=round( max(self.max_time - (time() - self.start_time), 0)), score=self.score
            )
        )
    ...
    def _human_step_env(self, joint_action):
        ...
        next_state, timestep_sparse_reward, done, info = self.env.step(joint_action, joint_agent_action_info =[{"1"},{"2"},{"3"}])
...
class StudyConfig:
    def __init__(self, args):
        ...
        self.layout_name = args.layout if args.num_players == 2 else '_'.join([args.layout, str(args.num_players)+'p'])
        ...
        path_from = os.path.join(
            os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir)), "src/data", "layout", layout_file_name)
        path_to = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir)), "overcooked_ai", "src", "overcooked_ai_py", "data", "layouts", layout_file_name)
        print(f'{path_from} to {path_to}')
        ...
        self.base_env = SteakhouseEnv.from_mdp(
            self.world_mdp, horizon=MAX_STEPS)
...
class Logger:
    ...
    def create_video(self, fps=10):
        image_files = sorted([img for img in os.listdir(self.img_dir) if img.endswith(".jpg") or img.endswith(".png")]) 
        clips = [os.path.join(self.img_dir, img) for img in image_files]
        clip = ImageSequenceClip(clips, fps=fps)
        clip.write_videofile(os.path.join(self.log_folder, self.filename+'.mp4'), codec='libx264')
...
def initialize_config_from_args():
    ...
    parser.add_argument('--num_players', type=int, default=2, help='Number of players in the game. Can be either 2 or 3 players.')
    ...
    if args.log_file_name == '':
        args.log_file_name = '-'.join([str(args.participant_id), args.layout, str(args.num_players)+'p'])
...
if __name__ == "__main__":
    ...
    gametime = study_config.total_time # 300 seconds
    gameapp = OvercookedPygame(study_config.base_env, agents, logger, gameTime=gametime)
    gameapp.on_execute()
