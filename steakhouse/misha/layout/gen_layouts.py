...
Three things fix it, and all three are structural rather than cosmetic:
  ...
  * NEITHER cook gets a private worktop. The old pass lined every wall-adjacent
    floor tile on the human's side and then converted each one back to '#' -
    which is where the ragged interior wall blobs came from - and laid the robot
    a budgeted BENCH of its own along its outer wall. Rule 3 now forbids both:
    the only staging surface either of them has is the PASS, which is what a
    pass is in a real kitchen. robot_bench() is kept below but no longer runs.
...
3. EVERY COUNTER IS REACHABLE FROM BOTH ROOMS.               counter_reach()
   Nothing either one puts down is ever out of the other's reach, so a handoff is
   never stranded and the only open question about a drop is whether the other
   one SAW it - which is what this suite exists to ask.
   This used to be one-directional, letting the robot keep a private bench since
   it can never serve. A counter only one of them can stand at is a STASH, not a
   handoff: putting an item there is a decision the other cook can neither see
   the point of nor act on, so it cannot produce a coordination failure and it
   gives the robot somewhere to dump an item that is not the pass. Both rooms
   reach every counter now, so every drop is a handoff offer.
...
def S_(name, w, h, axis, at, shape, split, why, mid=(), red=(), amp=3,
    ...
    bench and run are INERT: rule 3 forbids a private counter, so make() does not
    call robot_bench(). They are left on the specs rather than deleted so the
    bench can be switched back on for one suite without re-tuning sixteen specs.
    """
...
def robot_bench(rows, spec):
    ...
    NOT CALLED by make() any more - rule 3 wants every counter reachable from
    both rooms, and every tile this lays down is reachable from one. Kept whole,
    with its spec parameters, because "the robot may stash" is a question the
    suite may want to ask again, and it is the pass that answers it.
...
def drop_private_counters(rows):
    """RULE 3: a counter only ONE room can stand at stops being a counter.

    Where it goes back to depends on why it is not shared. A counter with floor
    beside it was floor before some pass paved it, so it goes back to ' ': floor
    and counter are both see-through, so reverting one changes no sight line and
    no visibility number, it only takes the surface away. A counter with no floor
    beside it at all was never standable, and turning THAT into floor would open
    a sealed third room, so it becomes '#'.

    With no bench pass and no human-side worktop this is a backstop rather than a
    pass that does work - which is the point, because when it DID do work it left
    a run of reverted counters behind as an interior wall blob.
    """
    g = [list(r) for r in rows]
    floor = _floor(rows)
    r_ok, h_ok = counter_reach(rows)
    for (x, y) in sorted(r_ok ^ h_ok):
        g[y][x] = " " if any(n in floor for n in _nbrs((x, y))) else "#"
    for c in stranded_counters(rows):
        g[c[1]][c[0]] = "#"
    return ["".join(r) for r in g]
...
def make(spec):
    ...
    The ORDER is forced, not a preference:
      ...
      drop_private_counters      rule 3 backstop. robot_bench() used to run just
                       before it and is now skipped: everything it laid down is
                       private by construction, so the pair cancelled out.
    ...
    if err:
        return None, err
    rows = drop_private_counters(rows)
    ...
    r_ct, h_ct = counter_reach(rows)
    if r_ct ^ h_ct:
        return None, "RULE 3: private counters %s" % sorted(r_ct ^ h_ct)
...
def audit_dir(d):
    ...
    for f in sorted(os.listdir(d)):
        ...
        out.append((f[:-len(".layout")], len(rows[0]), len(rows),
                    ...
                    bool(r_ct ^ h_ct), len(single_file(rows))))
    return out
...
if __name__ == "__main__":
    ...
    if arg == "--audit":
        ...
        print("%-14s %-8s %-10s %-14s %-4s %-4s %-6s %-6s %-8s %s"
              % ("layout", "size", "divide", "split", "mid", "red", "shared",
                 "spread", "watch", "robot | human"))
        for n, w, h, r in res:
            print("%-14s %3dx%-4d %-10s %-14s %-4s %-4s %-6d %-6.2f %-8s %s | %s%s"
