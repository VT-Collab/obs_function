...
class BayesianDelegationRobot:
    ...
    def __init__(self, mdp, agent_index=0, seed=0, beta=2.0, alpha=0.9,
        ...
        # rho is the STICKY-DRAW hold probability, unrelated to `beta` above --
        # see baselines.py's module docstring for the distribution and the
        # sticky kernel this robot's action() draws from. Named separately
        # because `beta` here already means something else (partner
        # rationality): this robot's draw needs no sharpness knob of its own,
        # because it samples its genuine posterior directly rather than lifting
        # a value function into one.
        self.rho = rho
        self.reset()
    ...
    #     rank_subtasks, none of the belief/allocation machinery this file
    #     deliberately keeps standalone from _BaseRobot, so sharing it
    #     costs nothing this file's independence actually protects.
    #     Imported lazily (inside the method, not at module scope) because
    #     baselines.py already imports GAMMA/TIER_GAIN FROM this module --
    ...
    def _short(sub):
        ...
        baselines.short_subtask, kept as its own copy -- see baselines.py's
        module docstring for why this file does not import it."""
        tier, verb, cell = sub
        return "%-4s %s@%d,%d" % (TIER_NAME[tier][:4], verb, cell[0], cell[1])

    def _pi(self):
        """{(tier, verb, cell): p}, sampled directly from this robot's own
        posterior rather than lifted from a value function -- see
        baselines.py's module docstring. IDLE and zero-mass entries are
        dropped, exactly as rank_subtasks() already drops IDLE, so the
        candidate set matches the ranking's and the DRAW is the only
        difference between them."""
        marg = self._marginal(0)
    ...
    def top3(pi, held):
        ...
        `held` is the realised draw, not argmax(pi): that is the whole point
        of drawing.
        """
        out = []
        for s, p in sorted(pi.items(), key=lambda kv: (-kv[1], kv[0]))[:3]:
            out.append((BayesianDelegationRobot._short(s), float(p), s == held))
        return out
    ...
    def _info(self, held, pi, ranked, why):
        ...
        return {"subtask": self.last_subtask,
                ...
                "top3_kind": "p",
                "partner_subtask": name, "partner_p": prob,
                "partner_entropy": self.partner_entropy(),
                "n_alloc": len(self.belief)}

    def action(self, state):
        """action() DRAWS its sub-task from its own posterior -- see
        baselines.py's module docstring for the sticky kernel every baseline
        in robot/nominal_policy/ draws through.

        `last_action`/`t`/`log` only update on a tick that actually draws and
        moves, not on the two early-return paths below (nothing ranked, or
        the posterior has no mass to draw from). Preserved as-is from this
        robot's previous StochasticSubtask wrapping, where those same two
        paths returned before reaching the code that touched them.
        """
        snap = self._snapshot(state)
        ...
        if forced:
            ...
            # Re-draw from the FULL pi, current sub-task included -- see
            # baselines.py's module docstring for why excluding it would break
            # stationarity.
