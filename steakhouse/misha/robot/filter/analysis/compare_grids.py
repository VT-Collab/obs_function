...
Mirrors no_larping/robot/filter/analysis/compare_grids.py, retargeted at this
package's fov-*-c8 pairings. Self-contained: no import from no_larping.

Paired win/tie/loss per pairing for both runs side by side, then every cell whose
result CHANGED.

TWO GUARDS AGAINST READING A DIFF THAT IS NOT ABOUT THE CODE:

  1. LAYOUT DRIFT. Rows carry `layout_sha` and a layout whose fingerprint
     differs is EXCLUDED from the tallies and listed separately.
...
PAIRS = [("solo", "fov-solo-c8"), ("greedy", "fov-greedy-c8"),
...
def main(argv=None):
    ...
    print("%-14s %-22s %s" % ("pairing", "OLD  win/tie/loss", "NEW  win/tie/loss"))
    print("-" * 62)
    for b, q in PAIRS:
        o, n = to[(b, q)], tn[(b, q)]
        print("%-14s %-22s %d/%d/%d" % (
    ...
    for b, q in PAIRS:
        for lay in layouts:
            for fov in FOVS:
                ...
                if vo != vn:
                    any_ = True
                    print("  %-14s %-13s fov%-4d %-5s -> %-5s   %.1f/%dt -> %.1f/%dt"
