...
than of where the furniture happens to sit. 'G' (glass) is the deliberate
counter-example: impassable like a wall (common/views.py's walkable sets check
for ' ', not '#' specifically, so any non-space char is already impassable),
but not '#', so it partitions the FLOOR without partitioning the FOV.
"""
import collections
