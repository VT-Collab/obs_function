#!/usr/bin/env python3
"""
LSTM (combo) — Predict ACTION from obs sequences.
...
Run:
  /Users/mishafu/miniconda3/envs/steakhouse-ai/bin/python lstm_combo_action.py
...
def main():
    CFG = dict(
        ...
        lr=1e-3,
        seed=7,
        project="10_10_steakhouse_combo",
        run_name="mixed_lstm_combo_action",
    )
    ...
    model  = LSTMModel(input_dim=input_dim, hidden_dim=CFG["hidden_dim"],
                       layer_dim=CFG["layer_dim"], output_dim=NUM_ACTIONS).to(device)
    ...
    for epoch in range(CFG["num_epochs"]):
        ...
        for xb, yb in train_loader:
            ...
            opt.zero_grad()
            logits, _, _ = model(xb)          # LSTMModel returns (logits, hn, cn)
            loss   = crit(logits, yb)
            loss.backward()
            opt.step()
            total += loss.item()
        ...
        wandb.log({
            "final/val_trueprob_timeseries":
                wandb.plot.line_series(xs=xs, ys=ys, keys=keys,
                                       title="Action (LSTM): true-class prob by timestep",
                                       xname="timestep")
        })
        ...
        wandb.log({
            "final/val_cumacc_by_timestep":
                wandb.plot.line_series(xs=xs2, ys=ys2, keys=keys2,
                                       title="Action (LSTM): cumulative accuracy by timestep",
                                       xname="timestep")
        })
    else:
        out_dir = PKL_DIR
        pd.DataFrame(prob_series).to_csv(os.path.join(out_dir, "lstm_action_val_trueprob_timeseries.csv"), index=False)
        pd.DataFrame(cum_lines).to_csv(os.path.join(out_dir, "lstm_action_val_cumacc_timeseries.csv"), index=False)
        print("[saved] CSVs in", out_dir)
        
    #MAY NEED TO CHANGE FROM MLP TO LSTM FUNCTIONALITIES
...
# ---- END Subtask "start-to-always-correct" tail percentage histogram (10% bins) ----
    ...
    # ==============================
    # DROP-IN MIDDLE-OF-EPISODE EVAL — LSTM ACTION
    # ==============================
    # if USE_WANDB:
    #     wandb.finish()
    ...
    #                                         title=f"Action LSTM drop-in @{tag}%", xname="timestep")})
    ...
    #                                         title=f"Action LSTM cum-acc drop-in @{tag}%", xname="timestep")})
