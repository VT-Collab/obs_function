#!/usr/bin/env python3
"""
MLP (combo) — Predict INTENT (12-way) from obs sequences.
...
Run:
  /Users/mishafu/miniconda3/envs/steakhouse-ai/bin/python mlp_combo_intent.py
...
# ---------- Intent space ----------
IDX_TO_INTENT = {
    0: 'pickup_meat', 1: 'pickup_onion', 2: 'pickup_dirty_plate',
    3: 'drop_meat',   4: 'drop_onion',   5: 'drop_dirty_plate',
    6: 'chop_onion',  7: 'rinse_plate',  8: 'pickup_clean_plate',
    9: 'pickup_steak',10: 'add_garnish', 11: 'deliver_dish',
}
...
def main():
    CFG = dict(
        ...
        run_name="mixed_mlp_combo_intent",
    )
    ...
    model  = MLP(seq_len=L, input_dim=input_dim, hidden_dim=CFG["hidden_dim"], output_dim=NUM_INTENTS).to(device)
        ...
        wandb.log({
            "final/val_trueprob_timeseries":
                wandb.plot.line_series(xs=xs, ys=ys, keys=keys,
                                       title="Intent: true-class prob by timestep (per episode)",
                                       xname="timestep")
        })
        ...
        wandb.log({
            "final/val_cumacc_by_timestep":
                wandb.plot.line_series(xs=xs2, ys=ys2, keys=keys2,
                                       title="Intent: cumulative accuracy by timestep (per episode)",
                                       xname="timestep")
        })
    else:
        out_dir = PKL_DIR
        pd.DataFrame(prob_series).to_csv(os.path.join(out_dir, "intent_val_trueprob_timeseries.csv"), index=False)
        pd.DataFrame(cum_lines).to_csv(os.path.join(out_dir, "intent_val_cumacc_timeseries.csv"), index=False)
        print("[saved] CSVs in", out_dir)
    
    # ==============================
    # DROP-IN MIDDLE-OF-EPISODE EVAL — MLP INTENT
    # ==============================
    # if USE_WANDB:
    #     wandb.finish()
    ...
    #             logits = model(seq.unsqueeze(0).to(device))  # MLP: logits directly
    ...
    #                                         title=f"Intent MLP drop-in @{tag}%", xname="timestep")})
    ...
    #                                         title=f"Intent MLP cum-acc drop-in @{tag}%", xname="timestep")})
