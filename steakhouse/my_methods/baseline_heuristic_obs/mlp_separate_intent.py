...
Run:
  /Users/mishafu/miniconda3/envs/steakhouse-ai/bin/python mlp_separate_intent.py
...
IDX_TO_INTENT = {
    0:'pickup_meat',1:'pickup_onion',2:'pickup_dirty_plate',3:'drop_meat',4:'drop_onion',5:'drop_dirty_plate',
    6:'chop_onion',7:'rinse_plate',8:'pickup_clean_plate',9:'pickup_steak',10:'add_garnish',11:'deliver_dish',
}
INTENT_TO_IDX = {v:k for k,v in IDX_TO_INTENT.items()}

WANDB_PROJECT="steakhouse_trueprob"
WANDB_ENTITY="steakteam"
RUN_NAME="mlp_separate_intent"

def to_intent_idx(row) -> Optional[int]:
    val = row.get("p0_subtask", row.get("p0_intent", None))
    if val is None: return None
    return INTENT_TO_IDX.get(str(val).strip(), None)

def load_pkl(path: str) -> pd.DataFrame:
    with open(path, "rb") as f: df = pickle.load(f)
    df["file_id"]=pathlib.Path(path).name
    df["episode"]=df["episode"].astype(int); df["timestep"]=df["timestep"].astype(int)
    ...
    for (fid, epi), g in df.groupby(["file_id","episode"], sort=False):
        ...
        for _, r in g.iterrows():
            ...
            try:
                ...
                continue
            vecs.append(v); intents.append(idx); ts.append(int(r["timestep"]))
        T = len(intents)
        if T <= L: continue
        for i in range(L, T):
...
def log_episode_lines(tag_prefix: str, rows: list):
    rows.sort(key=lambda d: (d["series"], d["timestep"]))
    from collections import defaultdict
    series_xy = defaultdict(lambda: {"x": [], "y": []})
    for r in rows:
        series_xy[r["series"]]["x"].append(r["timestep"])
        series_xy[r["series"]]["y"].append(r["trueprob"])
    keys=list(series_xy.keys()); xs=[series_xy[k]["x"] for k in keys]; ys=[series_xy[k]["y"] for k in keys]
    wandb.log({f"{tag_prefix}/per_episode_line": wandb.plot.line_series(xs=xs, ys=ys, keys=keys,
        title=f"{tag_prefix} — true prob by timestep", xname="timestep")})
    # avg
    from collections import defaultdict
    by_t=defaultdict(list)
    for r in rows: by_t[r["timestep"]].append(r["trueprob"])
    tbl=wandb.Table(columns=["timestep","avg_trueprob"])
    for t in sorted(by_t): tbl.add_data(t, float(np.mean(by_t[t])))
    wandb.log({f"{tag_prefix}/avg_by_timestep": wandb.plot.line(tbl, "timestep","avg_trueprob",
        title=f"{tag_prefix} — avg true prob by timestep")})

def main():
    random.seed(7); np.random.seed(7); torch.manual_seed(7)
    wandb.init(project=WANDB_PROJECT, entity=WANDB_ENTITY, name=RUN_NAME,
               config=dict(seq_length=3, batch_size=128, hidden_dim=128, num_epochs=200, lr=1e-3))
    CFG=wandb.config
    ...
    for train_path in TRAIN_FILES:
        if not os.path.exists(train_path): print(f"[skip] {train_path}"); continue
        tr_df=load_pkl(train_path)
        trX,trY,_=create_sequences_intent(tr_df,L)
        if not trX: print(f"[no seq] {train_path}"); continue

        x_tr=torch.stack(trX); y_tr=torch.stack(trY)
        D=int(x_tr.shape[-1])
        tr_loader=DataLoader(TensorDataset(x_tr,y_tr), batch_size=B, shuffle=True)

        model=MLP(seq_len=L, input_dim=D, hidden_dim=H, output_dim=len(INTENT_TO_IDX)).to(device)
        ce=nn.CrossEntropyLoss(); opt=torch.optim.Adam(model.parameters(), lr=LR)
        ...
        # post-training per-episode lines on each VAL file
        model.eval()
        for val_path in VAL_FILES:
            if not os.path.exists(val_path): continue
            va_df=load_pkl(val_path)
            vaX,vaY,vaM=create_sequences_intent(va_df,L)
            series=[]
            with torch.no_grad():
                for seq,lab,meta in zip(vaX,vaY,vaM):
                    p_true=float(gather_true_probs(model(seq.unsqueeze(0).to(device)), lab.view(1).to(device)).item())
                    fid,epi,t=meta
                    series.append({"series": f"{pathlib.Path(train_path).name}→{fid}|ep{epi}",
                                   "file_id":fid,"episode":int(epi),"timestep":int(t),"trueprob":p_true})
            tag=f"separate_intent/{pathlib.Path(train_path).name}/on/{pathlib.Path(val_path).name}"
            log_episode_lines(tag, series)
