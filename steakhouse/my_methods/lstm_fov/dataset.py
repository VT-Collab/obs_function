...
def obs_list_to_1D_vec(obs):
    ...
    USE_POTS = False
    if USE_POTS:
        flat += encode_station(obs[11], 'pots') #vector dim is 55 instead of 65 after removing pots
...
if __name__ == "__main__":
    ...
    dataset_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../data/fov120_Overcooked2_2-4")
    )
    pkl_path = os.path.join(dataset_dir, "120.pkl")
    csv_path = os.path.join(dataset_dir, "120.csv")

    # Load DF
    if os.path.exists(pkl_path):
        df = pd.read_pickle(pkl_path)
        src_used = pkl_path
        print(f"[test] loaded PKL: {pkl_path}")
    elif os.path.exists(csv_path):
        df = pd.read_csv(csv_path)
        src_used = csv_path
        print(f"[test] loaded CSV: {csv_path}")
    else:
        raise FileNotFoundError(f"Could not find 120.pkl or 120.csv in {dataset_dir}")

    if "obs" not in df.columns:
        raise KeyError("Column 'obs' not found in the dataset. Make sure your combine script exported it.")

    # Find first non-empty obs
    mask = df["obs"].notna() & df["obs"].astype(str).str.strip().ne("")
    if not mask.any():
        raise ValueError("No usable 'obs' values found in the dataset.")

    row = df.loc[mask].iloc[0]  # positional pick from filtered rows
    row_label = row.name
    obs_val = row["obs"]

    # Ensure obs is a STRING compatible with your existing obs_to_list (no changes to that function)
    def to_obs_string(val):
        if isinstance(val, str):
            return val
        if isinstance(val, list):
            # Convert each element to its string representation so obs_to_list can ast.literal_eval it
            # Example: [[11,5],[0,1],None,{...}] -> "['[11, 5]', '[0, 1]', 'None', '{...}']"
            return str([str(e) for e in val])
        # Fallback: plain str()
        return str(val)

    obs_raw = to_obs_string(obs_val)

    print(f"[test] using {src_used}; DataFrame index label = {row_label}")
    print(f"[test] obs string (truncated): {obs_raw[:120]}...")

    # Parse and encode without touching obs_to_list
    obs_list = obs_to_list(obs_raw)
    vec = obs_list_to_1D_vec(obs_list)

    print("[test] 1D vector:", vec)
    print("[test] length:", len(vec))
    if len(vec) == 65:
        print("[test] ✅ vector length is 65 (expected).")
    else:
        print(f"[test] ⚠️ vector length is {len(vec)} (expected 65). Check encoders & input format.")
