...
def main():
    ...
    print(f"WATCH {SCENE}: {len(road.vehicles)} vehicles (1 LimitedVisionHuman, fov={args.fov:.0f} "
    ...
    pygame.display.set_caption(f"WATCH -- LimitedVisionHuman on {SCENE} (fov={args.fov:.0f})")
    ...
    while running:
        ...
        window.blit(font.render(
            f"WATCH {SCENE}  step {step_count}  ({len(road.vehicles)} vehicles)  "
