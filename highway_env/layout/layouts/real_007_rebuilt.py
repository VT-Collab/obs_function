...
directly -- no second change needed, since every primitive from there on
...
def _build_network() -> RoadNetwork:
    ...
    stitch_junction(net, ("junction", "tw1_", 1, 0), "tw2_", TW2_ACCESS, N_TW2,
                     missing_corner=TW2_MISSING_CORNER, gap=GAP)

    # LEFT turn onto TW2's south arm (heading 90) -> roundabout, entering
    # from its north-facing gap. TW2 and the roundabout carry the SAME
    # lane count (both 2), so every lane bridges 1:1.
    stitch_roundabout(net, "tw2_", 0, 1, "ra_", RA_RADIUS, RA_ACCESS, RA_ALPHA, RA_MERGE_RADIUS,
                       n_lanes=N_TW2, gap=GAP, in_k=_RA_IN_K, bridge_lane_i=0)

    # Half-turn around the ring -> highway merge, bidirectional (matching
    ...
    # T-junctions and the roundabout -- carries the route to the merge's
    # own lane 1 without any further changes in between.
...
_SHARED_TAIL = [
    ...
    ("tw2_il0_1", "tw2_o0_1", 0), ("tw2_o0_1", f"ra_farin{_RA_IN_K}_1", 0),
    (f"ra_farin{_RA_IN_K}_1", f"ra_bendin{_RA_IN_K}_1", 0), (f"ra_bendin{_RA_IN_K}_1", f"ra_entry{_RA_IN_K}_1", 0),
    (f"ra_entry{_RA_IN_K}_1", f"ra_exit{_RA_HOP_K}_1", 0),
    (f"ra_exit{_RA_HOP_K}_1", f"ra_entry{_RA_HOP_K}_1", 0),  # pass through, don't leave
    (f"ra_entry{_RA_HOP_K}_1", f"ra_exit{_RA_OUT_K}_1", 0),
