"""Real scene 009, rebuilt: built entirely from build_scene.py's own
...
at one T-junction, a quarter-turn around a roundabout, a RIGHT turn at a
...
quarter-turn, and TW2, landing directly on the merge highway's own lane 1
...
TW2_ACCESS = 25.0
N_TW2 = 2
TW2_MISSING_CORNER = 2  # stem = corner 0 (south), matching the roundabout's own north exit
...
# Deterministic from the fixed geometry (TW1's left turn lands on heading
# 180, west) -- computed once here so _SHARED_TAIL can reference the same
# values _build_network uses.
_RA_IN_K = ring_gap_for_heading(np.pi)
...
def _build_network() -> RoadNetwork:
    ...
    # Quarter-turn around the ring -> TW2's south arm (stem). Queried on
    # lane 0 (stitch_junction's own roundabout-source branch bridges
    # starting at this SAME lane index, so it must be 0 to cover both
    # roundabout lanes {0, 1}).
    stitch_junction(net, ("roundabout", "ra_", _RA_OUT_K, 0), "tw2_", TW2_ACCESS, N_TW2,
                     missing_corner=TW2_MISSING_CORNER, gap=GAP)

    # RIGHT turn onto the east arm -> highway merge, bidirectional
    # (matching every other primitive here) and exact_center=True since
    # this bridges N_TW2 lanes in parallel.
    stitch_merge(net, ("junction", "tw2_", 3, 1), "mg_", MG_LANES, gap=GAP, exact_center=True,
...
_SHARED_TAIL = [
    ...
    ("tw1_il1_1", "tw1_o1_1", 0), ("tw1_o1_1", f"ra_farin{_RA_IN_K}_1", 0),
    ...
    ("tw2_o0_1", "tw2_ir0_1", 0), ("tw2_ir0_1", "tw2_il3_1", 0),  # right turn onto the east arm, lane 1
    ("tw2_il3_1", "tw2_o3_1", 0), ("tw2_o3_1", "mg_a_1", 0),      # bridge, into the highway's own normal lane 1
