"""Real scene 005, rebuilt: built entirely from build_scene.py's own
...
at one T-junction, a quarter-turn around a roundabout, a LEFT turn at a
second T-junction, and a highway on-ramp merge.
...
dropping back to lane 0 via the roundabout bridge itself, and one more
onto the merge highway's own lane 1, ridden to the end rather than changed
back, same as real_001_rebuilt.py (see that file's docstring on why an
earlier "out and back" pair on the merge highway left the outer lane
looking cut).
...
TW2_ACCESS = 25.0
N_TW2 = 2
TW2_MISSING_CORNER = 2  # stem = corner (2+2)%4 = 0 (south), matching the roundabout's own north exit
...
# LEFT turn from TW1's south stem always lands on its west arm, exit
# heading 180) -- computed once here so _SHARED_TAIL can reference the same
# values _build_network uses.
_RA_IN_K = ring_gap_for_heading(np.pi)  # 180 degrees, west
...
def _build_network() -> RoadNetwork:
    ...
    # LEFT turn onto the west arm (heading 180) -> roundabout, entering
    ...
    # Quarter-turn around the ring -> second T-junction, entering its south arm (stem).
    ...
    tw_center = center_ahead_corner(ra_out_far, ra_out_heading, 0, TW2_ACCESS, tw_od, GAP, LANE_WIDTH)
    ...
    bridge_roundabout(net, "ra_", _RA_OUT_K, "tw_", 0)

    # LEFT turn onto the west arm -> highway merge.
    tw_far, tw_heading = exit_far(net, "tw_", 1, 0)
    ...
    bridge(net, "tw_o1_0", "mg_a_0", tw_far, mg_center)
...
_SHARED_TAIL = [
    ...
    ("tw1_il1_1", "tw1_o1_1", 0), ("tw1_o1_1", f"ra_farin{_RA_IN_K}_0", 0),
    ...
    (f"ra_farout{_RA_OUT_K}_0", "tw_o0_0", 0),
    ("tw_o0_0", "tw_ir0_0", 0), ("tw_ir0_0", "tw_il1_0", 0),  # left turn onto the west arm
    ("tw_il1_0", "tw_o1_0", 0), ("tw_o1_0", "mg_a_0", 0),
