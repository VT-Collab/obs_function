...
a second T-junction, and a highway on-ramp merge on the opposite side
(ramp_side=-1).
...
(TW1<->RA and RA<->TW2 stay single-lane, since the roundabout itself only
has one). The route makes two lane changes total: one at FW's own exit
(0 -> 1), riding lane 1's own pre-built turn geometry through TW1 before
dropping back to lane 0 -- via the roundabout bridge itself, not a
separate change -- and one more onto the merge highway's own lane 1,
ridden to the end rather than changed back, same as real_001_rebuilt.py
(see that file's docstring on why an earlier "out and back" pair on the
merge highway left the outer lane looking cut).
...
def _build_network() -> RoadNetwork:
    ...
    tw_center = center_ahead_corner(fw_far, fw_heading, 0, TW1_ACCESS, tw_od, GAP, LANE_WIDTH)
    add_three_way(net, center=tw_center, n_stem=N_TW1, n_cross=N_TW1, access_length=TW1_ACCESS,
                  prefix="tw_", missing_corner=TW1_MISSING_CORNER)
    bridge_corners(net, "fw_", 2, "tw_", 0, N_FW)

    # RIGHT turn onto the east arm (heading 0) -> roundabout, entering from
    # its west-facing gap. Queried on lane 1 -- the route rides lane 1
    # through TW1 (see the lane change below) -- though the bridge itself
    # only carries the one lane the route uses.
    tw_far, tw_heading = exit_far(net, "tw_", 3, 1)
    ra_in_k = _RA_IN_K
    ...
    bridge_roundabout(net, "ra_", ra_in_k, "tw_", 3, j_lane_i=1)
    ...
    tw2_center = center_ahead_corner(ra_out_far, ra_out_heading, 0, TW2_ACCESS, tw2_od, GAP, LANE_WIDTH)
    ...
    bridge_roundabout(net, "ra_", _RA_OUT_K, "tw2_", 0)

    # LEFT turn onto the west arm -> highway merge on the opposite ramp side.
    tw2_far, tw2_heading = exit_far(net, "tw2_", 1, 0)
    ...
    bridge(net, "tw2_o1_0", "mg_a_0", tw2_far, mg_center)
...
_SHARED_TAIL = [
    ...
    ("tw_il3_1", "tw_o3_1", 0), ("tw_o3_1", f"ra_farin{_RA_IN_K}_0", 0),
    (f"ra_farin{_RA_IN_K}_0", f"ra_bendin{_RA_IN_K}_0", 0), (f"ra_bendin{_RA_IN_K}_0", f"ra_entry{_RA_IN_K}_0", 0),
    (f"ra_entry{_RA_IN_K}_0", f"ra_exit{_RA_HOP1_K}_0", 0),
    (f"ra_exit{_RA_HOP1_K}_0", f"ra_entry{_RA_HOP1_K}_0", 0),  # pass through, don't leave
    (f"ra_entry{_RA_HOP1_K}_0", f"ra_exit{_RA_HOP2_K}_0", 0),
    (f"ra_exit{_RA_HOP2_K}_0", f"ra_entry{_RA_HOP2_K}_0", 0),  # pass through, don't leave
    (f"ra_entry{_RA_HOP2_K}_0", f"ra_exit{_RA_OUT_K}_0", 0),
    (f"ra_exit{_RA_OUT_K}_0", f"ra_bendout{_RA_OUT_K}_0", 0), (f"ra_bendout{_RA_OUT_K}_0", f"ra_farout{_RA_OUT_K}_0", 0),
    (f"ra_farout{_RA_OUT_K}_0", "tw2_o0_0", 0),
    ("tw2_o0_0", "tw2_ir0_0", 0), ("tw2_ir0_0", "tw2_il1_0", 0),  # left turn onto the west arm
    ("tw2_il1_0", "tw2_o1_0", 0), ("tw2_o1_0", "mg_a_0", 0),
