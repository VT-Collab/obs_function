...
def _build_network() -> RoadNetwork:
    ...
    # every lane bridges 1:1 (see real_001_rebuilt.py's own docstring on
    # bridge_roundabout's own n>1).
    ...
    # Quarter-turn around the ring -> T-junction, entering its west arm
    ...
    # cover both roundabout lanes {0, 1} -- see stitch_junction's own
    # docstring).
    stitch_junction(net, ("roundabout", "ra_", _RA_OUT_K, 0), "tw1_", TW1_ACCESS, N_TW1,
                     missing_corner=3, gap=GAP)  # stem = corner (3+2)%4 = 1 (west)

    # LEFT turn onto the north arm (heading -90) -> second T-junction,
    # entering its south arm (stem).
    stitch_junction(net, ("junction", "tw1_", 2, 0), "tw2_", TW2_ACCESS, N_TW2,
    ...
    # N_TW2 lanes in parallel (see stitch_merge's own docstring).
    ...
    # geometry then carries the route through TW2 and onto the merge's own
    # lane 1 directly, without any further changes in between.
