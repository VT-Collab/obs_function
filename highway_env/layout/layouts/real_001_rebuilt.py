"""LOCKED -- do not edit this file. Its geometry is the validated reference
(pixel-verified against real_001_demo.png). Other layouts and tooling may
import/reference it (build_scene.py's stitch_* helpers, display_all.py,
etc.) but real_001's own construction here must never change. If a change
seems needed, prototype it in a new file first, get it validated, and only
then discuss touching this one -- see this repo's own memory notes on
real_001 (steakhouse-style "prototype in new file, then merge" convention).
The file is also chmod 444 on disk as a hard backstop.
...
def _build_network() -> RoadNetwork:
    ...
    # down the highway, but stops at "mg_c_1" -- the merge zone's own far
    # end -- rather than continuing through the after zone too.
