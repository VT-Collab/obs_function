...
ROBOT_ROUTE spawns right beside HUMAN_ROUTE's own start (a private bridge
lane straight to the T-junction's south arm, purely so both vehicles are
visible together from frame 1 -- see _build_network's own comment on it),
then takes the exact same straight-through movement onto that same
north-arm node the human's own turn lands on -- from there the two routes
are identical: turn, half the roundabout, and the whole merge stretch,
shared start to finish.
...
def _build_network() -> RoadNetwork:
    ...
    # A private spawn lane for the robot, right beside the human's own
    # spawn point instead of ~220m away at tw_'s south arm (where
    # ROBOT_ROUTE otherwise starts) -- so a viewer watching play.py/watch.py
    # sees both vehicles together from frame 1, not just once the human's
    # own route happens to reach the shared tail. Two fresh nodes
    # ("rb_spawn_0" and this lane), not a reuse of any existing junction
    # arm: reusing fw_'s own o1/o2 node would add a second outgoing edge at
    # a node background traffic can also land on (its own arm exit),
    # silently rerouting anonymous traffic across the whole bridge to tw_ --
    # this way nothing about either junction's own turn/fork logic changes,
    # only where the robot itself starts. Feeds directly into tw_o0_0 (the
    # robot's original entry point), so everything past this one new edge
    # -- the crossing convergence with the human's own left turn at tw_,
    # the shared tail -- is completely unchanged.
    human_spawn_lane = net.get_lane(("fw_o0_0", "fw_ir0_0", 0))
    rb_spawn_point = human_spawn_lane.position(8.0, 2 * LANE_WIDTH)
    tw_entry_point = net.get_lane(("tw_o0_0", "tw_ir0_0", 0)).position(0, 0)
    net.add_lane("rb_spawn_0", "tw_o0_0", StraightLane(rb_spawn_point, tw_entry_point, line_types=[C, C]))
...
def route_adjacent_lane_indexes(radius=15.0):
    ...
    route never comes near at all (the roundabout's unused east/west
    cardinals, the four-way's unused west arm, etc.) -- uniform spawning
    wastes a lot of `count` there instead of near the human, which is
    largely why an earlier FOV sweep on this layout showed no measurable
    difference between FOV widths: the human rarely encountered anything
    to actually miss. This lets a caller spawn background traffic that
    actually crosses or shares the human's own path."""
    net = _build_network()
    route_pts = np.array(HUMAN_ROUTE)
    result = []
